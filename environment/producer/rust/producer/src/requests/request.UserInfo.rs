...
use super::{ tools, ConsumersChannel, broadcasting, Broadcasting, ProducerEvents, Protocol };
...
pub trait UserInfo {
    ...
    fn listen<UCX: 'static + Sync + Send + Clone>(
        ...
        let (sender, receiver): (Sender<(Uuid, Protocol::UserInfo::Request)>, Receiver<(Uuid, Protocol::UserInfo::Request)>) = mpsc::channel();
        spawn(move || {
            loop {
                match receiver.recv() {
                    Ok((uuid, request)) => {
