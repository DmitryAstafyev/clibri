...
use super::{broadcasting, tools, ConsumersChannel, ProducerEvents};
                ...
                Ok(uuid) => match consumers.lock() {
                    Ok(consumers) => {
                        ...
                        if let Err(e) = Self::disconnected(uuid, ucx.clone()) {
                            if let Err(e) = feedback.send(ProducerEvents::EventError(
                                tools::logger.warn(&format!(
                                    "Call disconnected handler for event due error: {}",
                                    e
                                )),
                            )) {
                                tools::logger
