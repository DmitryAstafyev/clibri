    ...
    UCX: 'static + Sync + Send + Clone,
{
    fn listen<S: 'static + Interface + Sync + Send>(
        ...
        let server_listener_task: JoinHandle<Result<(), String>> = self.spawn_server_listener(
            rx_server_events,
            tx_consumers.clone(),
            tx_producer_events.clone(),
        );
        let server_rt_tast : JoinHandle<Result<(), String>> = self.spawn_server(
            server,
            tx_server_events,
            rx_sender,
            rx_server_control,
        );
        let consumers_task: JoinHandle<Result<(), String>> = self.spawn_consumers(
            tx_consumers,
            rx_consumers,
            tx_sender,
            tx_producer_events,
    ...
    fn spawn_server<S: 'static + Interface + Sync + Send>(
        &self,
        mut server: S,
                                ...
                                Ok(mut msgs) => {
                                    match msgs.UserDisconnected.1.pack(0, Some(uuid.to_string())) {
                                        Ok(buffer) => if let Err(e) = broadcast(msgs.UserDisconnected.0, buffer) {
                                            if let Err(e) = tx_producer_events.send(ProducerEvents::BroadcastingError(
                                                tools::logger.err(&format!("ConsumersChannel::Remove: cannot broadcast UserDisconnected due error: {}", e)),
                                            )) {
                                                tools::logger.err(&format!("{}", e));
                                            }
                                        },
                                        Err(e) => if let Err(e) = tx_producer_events.send(ProducerEvents::BroadcastingError(
