...
import { Logger, DefaultLogger, IServerReceivedEvent } from "clibri";
...
function test() {
	...
	server.getEvents().received.subscribe((event: IServerReceivedEvent) => {
		...
		if (!clients.has(event.uuid)) {
			...
			stats.errors.push(
				`Client ${event.uuid} Has been gotten unexpected data: ${event.buffer}`
			);
		} else {
...
function done() {
	...
	if (
		stats.disconnected === CLIENTS_COUNT &&
		stats.msdisconnected > 100000000
	) {
