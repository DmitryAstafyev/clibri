...
export class Server extends ServerInterface {
    ...
    constructor(addr: string, options: IServerOptions = {}) {
        ...
        this._logger = options.logger !== undefined ? options.logger : new DefaultLogger(`Server`);
    ...
    public listen(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                return reject(new Error(`Fail start server. Error: ${e.message}`));
            }
            this._server.addListener('error', this._onError.bind(this));
            this._server.addListener('connection', this._onConnection.bind(this));
            this._server.addListener('close', this._onClose.bind(this));
            resolve();
    ...
    public getStats(): {
        ...
        return {
            ...
            clients: this._server.clients.size,
        }
    }
    ...
    private _onConnection(socket: WebSocket, request: IncomingMessage): void {
        const connection: Connection = new Connection(socket, this._extLogger);
        connection.established().then(() => {
            this._logger.debug(`New connection established: ${connection.getGuid()}`);
            this._connections.set(connection.getGuid(), connection);
            connection.getSubjects().error.subscribe(this._onConnectionError.bind(this, connection));
            connection.getSubjects().data.subscribe(this._onConnectionData.bind(this, connection));
            connection.getSubjects().disconnected.subscribe(this._onConnectionDisconnected.bind(this, connection));
            this._subjects.connected.emit(connection.getGuid());
        }).catch((err: Error | IServerError) => {
            this._logger.warn(`Fail to add connection. Error: ${err.message}`);
    ...
    private _onConnectionError(connection: Connection, error: IServerError): void {
        this._subjects.error.emit(error);
        this._logger.warn(`Error on connection "${connection.getGuid()}". Error [${error.type}]: ${error.message}`);
    }
