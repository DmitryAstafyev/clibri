...
pub mod ServerEvents {
    ...
    impl StructDecode for UserKickOff {
        ...
        fn extract_from_storage(&mut self, mut storage: Storage) -> Result<(), String> {
            self.reason =
                match Option::<String>::get_from_storage(Source::Storage(&mut storage), Some(26)) {
                    Ok(val) => val,
                    Err(e) => return Err(e),
                };
...
pub mod Messages {
    ...
    impl StructDecode for Response {
        ...
        fn extract_from_storage(&mut self, mut storage: Storage) -> Result<(), String> {
            self.messages =
                match Vec::<Message>::get_from_storage(Source::Storage(&mut storage), Some(46)) {
                    Ok(val) => val,
                    Err(e) => return Err(e),
                };
            Ok(())
...
pub mod Users {
    ...
    impl StructDecode for Response {
        ...
        fn extract_from_storage(&mut self, mut storage: Storage) -> Result<(), String> {
            self.users =
                match Vec::<User>::get_from_storage(Source::Storage(&mut storage), Some(72)) {
                    Ok(val) => val,
                    Err(e) => return Err(e),
                };
            Ok(())
    ...
    fn get_msg(&self, id: u32, buf: &[u8]) -> Result<AvailableMessages, String> {
        match id {
            ...
            25 => match ServerEvents::UserKickOff::extract(buf.to_vec()) {
                Ok(m) => Ok(AvailableMessages::ServerEvents(
                    ServerEvents::AvailableMessages::UserKickOff(m),
                )),
                Err(e) => Err(e),
            },
