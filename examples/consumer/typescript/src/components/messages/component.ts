...
export class MessagesComponent extends Component {
	...
	public element(): HTMLElement {
		if (this._messages.length === 0) {
			...
			element.innerHTML = this._messages
				.map((msg: IMessage) => {
	...
	private _request() {
		new MessagesRequest(new Protocol.Messages.Request({}))
			.send()
			.then(
				(
					response: Protocol.Messages.Response | Protocol.Messages.Err
				) => {
