...
export class BeaconsLikeUser extends Protocol.Beacons.LikeUser {
    ...
    constructor(beacon: Protocol.Beacons.ILikeUser, consumer?: Consumer) {
        super(beacon);
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "BeaconsLikeUser" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "BeaconsLikeUser".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): BeaconsLikeUser {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): BeaconsLikeUser {
