...
export class Consumer {
	public static PROTOCOL_HASH: string =
		"F63F41ECDA9067B12F9F9CF312473B95E472CC39C08A02CC8C37738EF34DCCBE";
	public static WORKFLOW_HASH: string =
	...
	public static wait(handler: () => void): Error | Subscription {
		...
		return (global[Consumer.GUID_SUBS] as Subject<Consumer>).subscribe(
			handler
		);
	}
	...
	private readonly _pending: Map<
		number,
		(response: Protocol.IAvailableMessages) => void
	> = new Map();
	...
	public set uuid(value: string | undefined) {
		if (value === undefined) {
			...
			this._logger.err(
				`Fail set consumer UUID because value has invalid type (${typeof value}) or invalid value (${value})`
			);
	...
	constructor(
		client: Client,
		key: Protocol.Identification.ISelfKey,
		options?: IOptions
	) {
		this._client = client;
		this._key = key;
		...
		this._subscriptions.data = this._client
			.getEvents()
			.data.subscribe(this._onData.bind(this));
		this._subscriptions.connected = this._client
			.getEvents()
			.connected.subscribe(this._onClientConnected.bind(this));
		this._subscriptions.disconnected = this._client
			.getEvents()
			.disconnected.subscribe(this._onClientDisconnected.bind(this));
		this._subscriptions.error = this._client
			.getEvents()
			.error.subscribe(this._onClientError.bind(this));
	...
	public request(
		...
		sequence?: number
	): Promise<Protocol.IAvailableMessages> {
		if (sequence !== undefined && this._pending.has(sequence)) {
			return Promise.reject(
				new Error(
					this._logger.debug(
						`Request with sequence #${sequence} has been already sent and pending for response`
					)
				)
			);
		}
	...
	public assign(key: Protocol.Identification.ISelfKey): Promise<string> {
		return new Promise((resolve, reject) => {
			const request: Protocol.Identification.SelfKey =
				new Protocol.Identification.SelfKey(key);
			const sequence: number = this.getSequence();
			this.request(request.pack(sequence), sequence)
				.then((response: Protocol.IAvailableMessages) => {
					...
					if (
						response.InternalServiceGroup.SelfKeyResponse ===
						undefined
					) {
						return reject(
							new Error(
								this._logger.err(
									`Expecting message "InternalServiceGroup.SelfKeyResponse".`
								)
							)
						);
					}
					this.uuid =
						response.InternalServiceGroup.SelfKeyResponse.uuid;
					this._logger.debug(
						`Consumer is assigned with uuid ${this.uuid}`
					);
				...
				.catch((err: Error) => {
					reject(
						new Error(
							this._logger.err(
								`Fail assing consumer due error: ${err.message}`
							)
						)
					);
				});
	...
	private _hash(): Promise<void> {
		return new Promise((resolve, reject) => {
			const request: Protocol.InternalServiceGroup.HashRequest =
				new Protocol.InternalServiceGroup.HashRequest({
			...
			this.request(request.pack(sequence), sequence)
				.then((response: Protocol.IAvailableMessages) => {
					...
					if (
						response.InternalServiceGroup.HashResponse === undefined
					) {
						return reject(
							new Error(
								this._logger.err(
									`Expecting message "InternalServiceGroup.HashResponse".`
								)
							)
						);
					}
					if (
						response.InternalServiceGroup.HashResponse.error !==
						undefined
					) {
						reject(
							new Error(
				...
				.catch((err: Error) => {
					reject(
						new Error(
							this._logger.err(
								`Fail check consumer's hash due error: ${err.message}`
							)
						)
					);
				});
	...
	private _onClientConnected() {
		...
		this.assign(this._key)
			.then((uuid: string) => {
				this._hash()
					.then(() => {
						this._logger.debug(
							`Protocol and workflow hashes has been accepted`
						);
						this.ready.emit(uuid);
					})
					.catch((err: Error) => {
						this._logger.err(
							`Consumer has isn't accepted: ${err.message}\n\t- protocol hash: ${Consumer.PROTOCOL_HASH}\n\t- workflow hash: ${Consumer.WORKFLOW_HASH}`
						);
					});
			})
			.catch((err: Error) => {
				this._logger.err(
					`Default assign prodecure is failed: ${err.message}`
				);
			});
		this.connected.emit();
	}
	...
	private _onData(buffer: ArrayBufferLike) {
		...
		if (errors instanceof Array) {
			this._logger.err(
				`Fail to process chunk of data due error(s):\n\t${errors
					.map((e) => e.message)
					.join("\n\t")}`
			);
			return;
			...
			if (pending !== undefined) {
				...
				switch (id) {
					case Protocol.Events.UserConnected.getId():
						this.broadcast.EventsUserConnected.emit(
							msg.getRef<Protocol.Events.UserConnected>()
						);
						break;
					case Protocol.Events.Message.getId():
						this.broadcast.EventsMessage.emit(
							msg.getRef<Protocol.Events.Message>()
						);
						break;
					case Protocol.Events.UserDisconnected.getId():
						this.broadcast.EventsUserDisconnected.emit(
							msg.getRef<Protocol.Events.UserDisconnected>()
						);
						break;
					default:
						this._logger.warn(
							`Has been gotten unexpected message ID=${id};`
						);
						break;
				}
			}
