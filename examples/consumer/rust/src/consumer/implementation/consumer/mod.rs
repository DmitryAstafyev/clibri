                                    ...
                                    while let Some(msg) = buffer.next() {
                                        ...
                                        match msg.msg {                                                    
                                                    protocol::AvailableMessages::Events(protocol::Events::AvailableMessages::UserConnected(msg)) => {
                                                        tx_consumer_event.send(Emitter::EventsUserConnected(msg)).map_err(|e| ConsumerError::APIChannel(e.to_string()))?;
                                                    },
                                                    protocol::AvailableMessages::Events(protocol::Events::AvailableMessages::Message(msg)) => {
                                                        tx_consumer_event.send(Emitter::EventsMessage(msg)).map_err(|e| ConsumerError::APIChannel(e.to_string()))?;
                                                    },
                                                    protocol::AvailableMessages::Events(protocol::Events::AvailableMessages::UserDisconnected(msg)) => {
                                                        tx_consumer_event.send(Emitter::EventsUserDisconnected(msg)).map_err(|e| ConsumerError::APIChannel(e.to_string()))?;
                                                    },
                                            _ => {
                                                shortcuts::emit_error::<E>(
