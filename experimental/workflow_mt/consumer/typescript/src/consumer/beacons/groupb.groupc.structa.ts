...
export class GroupBGroupCStructA extends Protocol.GroupB.GroupC.StructA {
    ...
    constructor(beacon: Protocol.GroupB.GroupC.IStructA) {
        super(beacon);
    }
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "GroupBGroupCStructA" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "GroupBGroupCStructA".`
                                )
                            );
                    }
                })
