...
export class GroupAStructA extends Protocol.GroupA.StructA {
    ...
    constructor(beacon: Protocol.GroupA.IStructA) {
        super(beacon);
    }
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "GroupAStructA" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "GroupAStructA".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): GroupAStructA {
