use super::{
    hub, identification, pack, producer::Control, protocol, responses, Context, HandlerError,
    ProducerError,
};
use clibri::server;
...
pub async fn process<E: server::Error, C: server::Control<E> + Send + Clone>(
    ...
    let buffer =
        match responses::structf::response(identification, filter, context, request, control).await
        {
