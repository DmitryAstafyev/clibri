...
export class Producer {
	...
	constructor(server: Server, context: Context, options?: Options) {
		...
		this._subscriptions.eventsEventAHandlerSub =
			this.events.eventsEventA.subscribe(
		...
		this._subscriptions.eventsEventBHandlerSub =
			this.events.eventsEventB.subscribe(
		...
		this._subscriptions.eventsSubEventAHandlerSub =
			this.events.eventsSubEventA.subscribe(
		...
		this._subscriptions.triggerBeaconsEmitterHandlerSub =
			this.events.triggerBeaconsEmitter.subscribe(
		...
		this._subscriptions.finishConsumerTestHandlerSub =
			this.events.finishConsumerTest.subscribe(
	...
	private _onClientReceived(event: IServerReceivedEvent) {
		...
		do {
			...
			let extracted = this._extract(message.msg, "StructA");
			if (extracted.exist && !consumer.isHashAccepted()) {
				...
				const assignedUuid = consumer
					.getIdentification()
					.key(extracted.body<Protocol.StructA>(), true);
				this.send(
					assignedUuid,
