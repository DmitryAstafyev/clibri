...
export class StructD extends Protocol.StructD {
    ...
    constructor(request: Protocol.IStructD, consumer?: Consumer) {
        super(request);
                ...
                switch (this._state) {
                    case ERequestState.Pending:
                        ...
                        if (message === undefined) {
                            return reject(new Error(`Expecting message for "StructD".`));
                        } else if (message !== undefined && message.StructA !== undefined) {
                            this._handlers.response !== undefined && this._handlers.response(message.StructA);
                            return resolve(message.StructA);
                        } else if (message.StructC !== undefined) {
                            this._handlers.err !== undefined && this._handlers.err(message.StructC);
                            return resolve(message.StructC);
                        } else {
                            return reject(new Error(`No message in "message" group.`));
                        }
                    case ERequestState.Destroyed:
                        return reject(new Error(`Request "StructD" has been destroyed. Response would not be processed.`));
                    case ERequestState.Pending:
                        return reject(new Error(`Unexpected state for request "StructD".`));
                }
            }).catch((err: Error) => {
                reject(err);
            });
