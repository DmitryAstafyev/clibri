            ...
            consumer.request(this.pack(sequence), sequence).then((message: Protocol.IAvailableMessages) => {
                switch (this._state) {
                    ...
                    case ERequestState.Destroyed:
                        return reject(new Error(`Request "StructF" has been destroyed. Response would not be processed.`));
                    case ERequestState.Pending:
                        return reject(new Error(`Unexpected state for request "StructF".`));
                }
            }).catch((err: Error) => {
                reject(err);
            });
