...
export type TStructAResolver = Protocol.StructE | Protocol.StructB | Protocol.StructC | Protocol.StructD;
            ...
            consumer.request(this.pack(sequence), sequence).then((message: Protocol.IAvailableMessages) => {
                switch (this._state) {
                    ...
                    case ERequestState.Destroyed:
                        return reject(new Error(`Request "StructA" has been destroyed. Response would not be processed.`));
                    case ERequestState.Pending:
                        return reject(new Error(`Unexpected state for request "StructA".`));
                }
            }).catch((err: Error) => {
                reject(err);
            });
