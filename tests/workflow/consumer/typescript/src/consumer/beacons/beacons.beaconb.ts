...
export class BeaconsBeaconB extends Protocol.Beacons.BeaconB {
    ...
    constructor(beacon: Protocol.Beacons.IBeaconB, consumer?: Consumer) {
        super(beacon);
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "BeaconsBeaconB" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "BeaconsBeaconB".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): BeaconsBeaconB {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): BeaconsBeaconB {
