...
export class BeaconA extends Protocol.BeaconA {
    ...
    constructor(beacon: Protocol.IBeaconA, consumer?: Consumer) {
        super(beacon);
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "BeaconA" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "BeaconA".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): BeaconA {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): BeaconA {
