...
export class BeaconsBeaconA extends Protocol.Beacons.BeaconA {
    ...
    constructor(beacon: Protocol.Beacons.IBeaconA, consumer?: Consumer) {
        super(beacon);
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "BeaconsBeaconA" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "BeaconsBeaconA".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): BeaconsBeaconA {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): BeaconsBeaconA {
