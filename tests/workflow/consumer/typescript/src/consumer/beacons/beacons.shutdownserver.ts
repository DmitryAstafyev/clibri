...
export class BeaconsShutdownServer extends Protocol.Beacons.ShutdownServer {
    ...
    constructor(beacon: Protocol.Beacons.IShutdownServer, consumer?: Consumer) {
        super(beacon);
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "BeaconsShutdownServer" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "BeaconsShutdownServer".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): BeaconsShutdownServer {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): BeaconsShutdownServer {
