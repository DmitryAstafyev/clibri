...
export class BeaconsSubBeaconA extends Protocol.Beacons.Sub.BeaconA {
    ...
    constructor(beacon: Protocol.Beacons.Sub.IBeaconA, consumer?: Consumer) {
        super(beacon);
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "BeaconsSubBeaconA" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "BeaconsSubBeaconA".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): BeaconsSubBeaconA {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): BeaconsSubBeaconA {
