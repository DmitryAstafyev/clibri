...
export class StructB extends Protocol.StructB {
    ...
    constructor(beacon: Protocol.IStructB) {
        super(beacon);
    }
                ...
                .then((response: Protocol.IAvailableMessages) => {
                    switch (this._state) {
                        ...
                        case ERequestState.Destroyed:
                            return reject(
                                new Error(
                                    `Request "StructB" has been destroyed. Response would not be processed.`
                                )
                            );
                        case ERequestState.Pending:
                            return reject(
                                new Error(
                                    `Unexpected state for request "StructB".`
                                )
                            );
                    }
                })
    ...
    public response(handler: TResponseHandler): StructB {
        this._handlers.response = handler;
        return this;
    }

    public err(handler: TErrHandler): StructB {
